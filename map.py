...
class Map():
    def __init__(self, OCEANSIZE, ISLANDS, ISLAND_MIN_SIZE, ISLAND_MAX_SIZE):
        ...
        self.write_island_id_island_map()
        self.write_basic_island_map()

    def write_island_id_island_map(self):
        """
        Writes a basic ASCII-art map showing water and islands to the file islandmap.txt
        Different ASCII characters are used to represent each island, unless more than 95 islands are used.
        In this case, some characters may be used twice
        """
    ...
    def write_basic_island_map(self):
        """
        Writes a basic ASCII-art map of water and islands in text format to the file basicmap.txt
        """
        map_data = numpy.zeros((self.OCEANSIZE, self.OCEANSIZE))
        for island in self.islands_list.return_islands_list():
            island_full_coords = island.provide_coords()
